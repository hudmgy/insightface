...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
        ...
        if self.images_per_identity > 0:
            ...
            self.repeat = 3000000.0 / \
                (self.images_per_identity*len(self.id2range))
            ...
            for j in xrange(1, nrof_images):
                ...
                for pair in xrange(j, nrof_images):
                    ...
                    all_neg = np.where(np.logical_and(
                        neg_dists_sqr-pos_dist_sqr < self.triplet_alpha, pos_dist_sqr < neg_dists_sqr))[0]  # FaceNet selection
    ...
    def select_triplets(self):
        ...
        while len(self.seq) < self.seq_min_size:
            ...
            triplets = self.pick_triplets(
                embeddings, nrof_images_per_class)  # shape=(T,3)
    ...
    def hard_mining_reset(self):
        ...
        while ba < len(self.oseq):
            ...
            if X is None:
                X = np.zeros(
                    (len(self.id2range), nembedding.shape[1]), dtype=np.float32)
        ...
        for i in xrange(X.shape[0]):
            ...
            for _label in nnlist:
                ...
                if len(_list) < self.images_per_identity:
                    ...
                    _list = np.random.choice(
                        _list, self.images_per_identity, replace=False)
