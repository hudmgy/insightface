...
def fcnet(num_classes, filter_list, **kwargs):
    ...
    for ind, num_node in enumerate(filter_list):
        ...
        layer = mx.sym.LeakyReLU(data=layer, act_type='relu', name='layer%d_relu'%ind)

    fc1 = mx.sym.FullyConnected(data=layer, num_hidden=num_classes, name='pre_fc1')
    fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, eps=2e-5, momentum=0.9, name='fc1')
    fc1 = mx.sym.LeakyReLU(data=fc1, act_type='relu', name='fc1_relu')
...
def get_symbol(num_classes, num_layers, **kwargs):
    if num_layers==0:
        ...
        filter_list=[20,20,10]
    else:
        raise ValueError("no experiments done on num_layers {}, you can do it yourself".format(num_layers))

    return fcnet(num_classes = num_classes,
                  num_layers = filter_list,
                  **kwargs)
